...
function Activities({ lang, navigateTo ,userType}) {
  const memoryGames = [
    {
  ...
  const memoryGamesFam = [
    ...
    },
    {
      ...
      description1: lang === "en" 
        ? "" 
        : "اقلب البطاقات وطابق الأزواج لتنشيط عقلك وتحسين  ",
      image: "https://cdn-icons-png.flaticon.com/512/2103/2103633.png",
      page: "memorygame",
    }
  ];
  ...
  return (
    <div dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <div className="container mt-5">
        ...
        <p className="text-center text-muted mb-5" style={{ fontSize: '1.1rem' }}>
          {lang === "en" 
            ? "Choose an activity to follw the progress" 
            : "اختر النشاط لمتابعة التقدم"}
        </p>
          ...
          {memoryGames.map((game, index) => (
            <div key={index} className="col-md-6 col-lg-5 mb-4">
              ...
              >
                ...
                <div className="card-body">
                  <h5 className="card-title">{game.title}</h5>
                  <p className="card-description">{game.description1}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="memory-activities-tip">
          <div className="tip-text">
            <i className="bi bi-lightbulb-fill"></i>
            {lang === "en" 
              ? "Tip: Regular follow-up is good for the patient's health!" 
              : "نصيحة:  المتابعة بانتظام جيدة من اجل صحة المريض !"}
          </div>
        </div>
      </div>
    </div>
